package friendly.markdowntext.plugins.core
...
class MardownCorePlugin(
    ...
    override fun addOnTextAddedListener(
        onTextAddedListener: OnTextAddedListener,
    ): CorePlugin {
    ...
    private fun image(builder: MarkwonVisitor.Builder) {
        builder.on(
            Image::class.java,
            MarkwonVisitor.NodeVisitor { visitor, image ->
                ...
                visitor.setSpans(
                    length,
                    spanFactory.getSpans(configuration, props),
                )
            },
        )
    }
    ...
    private fun listItem(builder: MarkwonVisitor.Builder) {
        builder.on(
            ...
            if (parent is OrderedList) {
                ...
                CoreProps.LIST_ITEM_TYPE[visitor.renderProps()] =
                    CoreProps.ListItemType.ORDERED
                CoreProps.ORDERED_LIST_ITEM_NUMBER[visitor.renderProps()] =
                    start
    ...
    private fun paragraph(builder: MarkwonVisitor.Builder) {
        builder.on(
            ...
            CoreProps.PARAGRAPH_IS_IN_TIGHT_LIST[visitor.renderProps()] =
                inTightList
